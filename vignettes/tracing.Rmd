...
In a pipeline, each agent becomes its own span. This makes it easy to see
where time is spent across the chain:
...
worker execution gets its own span:
...
monitor spend across an entire graph run:
...
`vignette("cloud-native", package = "securetrace")` for full details.
...
Expose agent metrics for Prometheus scraping. Each traced graph run feeds
counters for span counts, token usage, and cost:
...
In production, combine OTLP, Prometheus, and JSONL for complete observability:
...
it. This means failed runs still produce useful traces:
